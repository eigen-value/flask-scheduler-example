...
var div=$('<div></div>').addClass(classes.sLength);if(!settings.aanFeatures.l){div[0].id=tableId+'_length'}
var controlPanel=document.getElementById('control-panel');if(controlPanel!=null){div.append(controlPanel.outerHTML);controlPanel.outerHTML=''}
div.append('<label/>');div.children('label').append(settings.oLanguage.sLengthMenu.replace('_MENU_',select[0].outerHTML));$('select',div).val(settings._iDisplayLength).on('change.DT',function(e){_fnLengthChange(settings,$(this).val());_fnDraw(settings)});$(settings.nTable).on('length.dt.DT',function(e,s,len){if(settings===s){$('select',div).val(len)}});return div[0]}
